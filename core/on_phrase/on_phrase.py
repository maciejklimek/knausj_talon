...
# def display_phrase_caption(phrase: Phrase):
#     """Display the recognized phrase as a caption at the bottom of the screen.
    
#     Note: The pre:phrase event is disabled as a workaround for a bug with dual monitors
#     where captions would not display at all. Using post:phrase ensures the
#     captions are properly displayed on the screen.
#     """
#     print(" ".join(phrase["phrase"]))
#     if phrase.get("phrase") and actions.speech.enabled():
#         text = " ".join(phrase["phrase"])
#         actions.user.display_notification(text, duration=3.0, font_size=96, height_fraction=0.9)
