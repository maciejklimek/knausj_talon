...
class Actions:
    ...
    def foot_switch_down_event(key: int):
        ...
        time_before = timestamps[key] if key < len(timestamps) else None
        state_before = current_state[key] if key < len(current_state) else None
        # print(f"[DEBUG] foot_switch_down_event: key={key}, time={time_before}, state before={state_before}")
        timestamps[key] = time.perf_counter()
        current_state[key] = DOWN
        # print(f"[DEBUG] foot_switch_down_event: key={key}, time={timestamps[key]}, state after={current_state[key]}")

    def foot_switch_up_event(key: int):
        """Foot switch key up event. Left(0), Center(1), Right(2), Top(3)"""
        state_before = current_state[key] if key < len(current_state) else None
        # print(f"[DEBUG] foot_switch_up_event: key={key}, state before={state_before}")
        current_state[key] = UP
        # print(f"[DEBUG] foot_switch_up_event: key={key}, state after={current_state[key]}")

    # Foot switch button actions. Modify these to change button behavior.

    def foot_switch_top_down():
        """Foot switch button top:down"""
        print("[DEBUG] foot_switch_top_down triggered")
        actions.user.mouse_scroll_down_continuous()
        print("[DEBUG] foot_switch_top_down ended")
        

    def foot_switch_top_up(held: bool):
        """Foot switch button top:up"""
        print(f"[DEBUG] foot_switch_top_up triggered, held={held}")
        if held:
            actions.user.mouse_scroll_stop()

    def foot_switch_center_down():
        """Foot switch button center:down"""
        print("[DEBUG] foot_switch_center_down triggered")
        actions.user.mouse_scroll_up_continuous()

    def foot_switch_center_up(held: bool):
        """Foot switch button center:up"""
        print(f"[DEBUG] foot_switch_center_up triggered, held={held}")
        if held:
            actions.user.mouse_scroll_stop()

    def foot_switch_left_down():
        """Foot switch button left:down"""
        print("[DEBUG] foot_switch_left_down triggered")
        global cron_job
        cron_job = cron.after(
    ...
    def foot_switch_left_up(held: bool):
        ...
        print(f"[DEBUG] foot_switch_left_up triggered, held={held}")
        global cron_job
